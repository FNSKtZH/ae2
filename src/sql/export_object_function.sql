CREATE OR REPLACE FUNCTION ae.export_object(export_taxonomies text[], tax_filters tax_filter[], pco_filters pco_filter[], rco_filters rco_filter[])
  ...
  $$
    ...
    WHERE
        ae.object.id IN (
            SELECT DISTINCT object_id FROM ae.export_pco(export_taxonomies, tax_filters, pco_filters)
            UNION SELECT DISTINCT object_id FROM ae.export_rco(export_taxonomies, tax_filters, rco_filters)
        )
  $$
  LANGUAGE sql STABLE;
