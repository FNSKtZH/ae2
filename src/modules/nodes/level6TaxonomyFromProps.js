...
export default ({
  ...
  if (!data) return []
  if (!data.level6Taxonomy) return []
  if (!data.level6Taxonomy.objectsByParentId) return []
  if (!data.level6Taxonomy.objectsByParentId.nodes) return []

  return data.level6Taxonomy.objectsByParentId.nodes.map(node => {
    const childrenCount =
